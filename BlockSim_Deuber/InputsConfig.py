...
class InputsConfig:
    ...
    if model == 1:
        ...
        NODES = [Node(id=0, hashPower=50), Node(id=1, hashPower=0), Node(id=2, hashPower=0),
                 ...
                 Node(id=6, hashPower=0), Node(id=7, hashPower=100), Node(id=8, hashPower=50),
                 Node(id=9, hashPower=100),Node(id=10, hashPower=0), Node(id=11, hashPower=200),
                 Node(id=12, hashPower=100), Node(id=13, hashPower=10), Node(id=14, hashPower=100)]

        Proposers = [node for node in NODES if (node.id == 0)] #or node.id ==6
        ''' Simulation Parameters '''
        simTime = 100000  # the simulation length (in seconds)
        Runs = 1  # Number of simulation runs
        VotePeriod = 15 # voting is collected over blocks
        RHO = 8  # voting is collected over blocks
