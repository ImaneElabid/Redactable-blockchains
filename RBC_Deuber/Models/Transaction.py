...
class LightTransaction():
    ...
    def create_candidate_block(node):
        Redacted_block= random.randint(0,len(node.blockchain)-1)
        proposer = random.randint(1, p.nodes)

        LightTransaction.redaction_candidate_tx = []
        pool = LightTransaction.redaction_candidate_tx
        Psize = int(p.transaction_rate * p.Binterval)  # (The nbr of tx to be created per s / time (in s) creating a block)
